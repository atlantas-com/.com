...
var DEV = (function(){
  ...
  var SECTION_META = {
    beta:         {title:'Beta Access',          sub:'Grant or revoke early access for specific users'},
    ...
    lock:         {title:'Lock Screen',        sub:'Payment methods, amounts, messages and contact for locked accounts'},
  ...
  function boot(){
    ...
    _auth.onAuthStateChanged(function(user){
      ...
      var isRootDev=(email===(DEV_EMAIL||'developer@gmail.com').toLowerCase());
      var isAdminEmail=(ADMIN_EMAIL&&email===ADMIN_EMAIL.toLowerCase());
      var isSubadmin=(SUBADMIN_DOMAIN&&email.endsWith(SUBADMIN_DOMAIN.toLowerCase()));
      if(isRootDev||isAdminEmail||isSubadmin){_showPortal(user);return;}
      _db.ref(DB.admins+'/'+user.uid).once('value',function(snap){
        var d=snap.val();
        if(d&&(d.isDev||d.isSuperAdmin)){_showPortal(user);return;}
        _showComingSoon(user);
      });
    });
  }
  ...
  function _showLogin(){
    ...
    document.getElementById('dev-coming-soon').style.display='none';
    document.getElementById('dev-login-btn').textContent='Access Developer Portal';
    document.getElementById('dev-login-btn').disabled=false;
  }
  ...
  function _showComingSoon(user){
    document.getElementById('dev-login').classList.remove('show');
    document.getElementById('dev-portal').classList.remove('show');
    document.getElementById('dev-coming-soon').style.display='flex';
  }
  ...
  function _loadBetaUsers(){
    ...
    _db.ref(DB.admins).once('value',function(snap){
      ...
      rows.forEach(function(uid){
        ...
        row.innerHTML=
          '<div><div style="font-size:13px;font-weight:600;color:var(--text);">'+_esc(entry.betaEmail||uid)+'</div>'+
          '<div style="font-size:11px;color:var(--text3);margin-top:2px;">UID: '+_esc(uid)+'</div></div>'+
          '<button onclick="DEV.betaRevoke(\''+_esc(uid)+'\',this)" style="background:rgba(248,113,113,0.08);border:1px solid rgba(248,113,113,0.2);color:var(--error);border-radius:8px;padding:6px 14px;font-size:12px;font-weight:600;cursor:pointer;">Revoke</button>';
        con.appendChild(row);
      });
    });
  }
  ...
  function betaLookup(){
    var email=(_getVal('beta-search-email')||'').trim().toLowerCase();
      ...
      _db.ref(DB.admins+'/'+uid+'/betaAccess').once('value',function(aSnap){
        ...
        res.innerHTML=
          '<div style="background:var(--bg3);border:1px solid var(--border2);border-radius:12px;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;">'+
            '<div>'+
              '<div style="font-size:14px;font-weight:700;color:var(--text);">'+_esc(name||email)+'</div>'+
              '<div style="font-size:12px;color:var(--text3);margin-top:2px;">'+_esc(email)+' · '+_esc(uid)+'</div>'+
              '<div style="font-size:12px;margin-top:6px;color:'+(hasAccess?'var(--success)':'var(--text2)')+';">'+
                (hasAccess?'✅ Has early access':'⏳ No access — Coming Soon')+'</div>'+
            '</div>'+
            (hasAccess
              ? '<button onclick="DEV.betaRevoke(\''+_esc(uid)+'\',this)" style="background:rgba(248,113,113,0.08);border:1px solid rgba(248,113,113,0.2);color:var(--error);border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;cursor:pointer;">Revoke</button>'
              : '<button onclick="DEV.betaGrant(\''+_esc(uid)+'\',\''+_esc(email)+'\',this)" style="background:linear-gradient(135deg,var(--p),var(--p2));color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;cursor:pointer;">Grant Access</button>'
            )+
          '</div>';
      });
    });
  ...
  function betaGrant(uid,email,btn){
    if(btn){btn.textContent='Saving…';btn.disabled=true;}
    _db.ref(DB.admins+'/'+uid).update({betaAccess:true,betaEmail:email},function(err){
      if(err){_showToast('Failed: '+err.message,'error');if(btn){btn.textContent='Grant Access';btn.disabled=false;}return;}
      _showToast('✅ Access granted to '+email,'success');
      betaLookup();
      _loadBetaUsers();
    });
  }

  function betaRevoke(uid,btn){
    if(btn){btn.textContent='Revoking…';btn.disabled=true;}
    _db.ref(DB.admins+'/'+uid+'/betaAccess').remove(function(err){
      if(err){_showToast('Failed: '+err.message,'error');if(btn){btn.textContent='Revoke';btn.disabled=false;}return;}
      _showToast('Access revoked.','success');
      var res=document.getElementById('beta-lookup-result');if(res)res.innerHTML='';
      _loadBetaUsers();
    });
  }
